...
<?php
	...
	if(!empty($_SESSION["uname"]))
	{
		...
		if($var==0)
		{
			...
			{
				$sql = "SELECT * FROM tasks WHERE assignedtoid=".$_SESSION["uid"];
				$result = mysqli_query($conn, $sql);
				$row = mysqli_fetch_assoc($result);	
			}
			if($_SESSION["user_type"]=="customer")
			{
				$sql = "SELECT * FROM tasks WHERE user_id=".$_SESSION["uid"];
				$result = mysqli_query($conn, $sql);
				$row = mysqli_fetch_assoc($result);	
			}
		}
		...
		{
			$sql = "SELECT * FROM tasks WHERE Category='database solutions'";
		}
		else if($var==5)
		{
		...
		if($_SESSION["user_type"]=='customer')
		{
			...
			{
				echo "<div class='jobs'> <p style='position:relative;  left:300px; top:50px; font-size: 30px;'>No Bugs posted by you. Post your Bugs now!</div>";
